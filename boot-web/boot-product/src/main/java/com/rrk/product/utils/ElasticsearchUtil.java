...
//                .size(pageSize);
     ...
     * @return
     */
    public static List<HotWordEntity> getHotWordList(String hotIndexName, QueryBuilder queryBuilder,
                                                     TermsAggregationBuilder aggregationBuilder) {
        SearchRequest searchRequest = new SearchRequest(hotIndexName);
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
        //设置搜索超时的时间
        searchSourceBuilder.timeout(new TimeValue(60, TimeUnit.SECONDS));
        ...
        if (searchResponse.status().getStatus() == 200) {
            Terms termAggs = searchResponse.getAggregations().get("productname");
            List<? extends Terms.Bucket> buckets = termAggs.getBuckets();
            for (Terms.Bucket bucket : buckets) {
                HotWordEntity wordEntity = new HotWordEntity();
                wordEntity.setProductName(bucket.getKey().toString());
                wordEntity.setQueryCount(Convert.toLong(bucket.getDocCount()));
                list.add(wordEntity);
            }
        }
        return list;
     ...
     *
     * @param indexName
     * @param suggestBuilder
     * @return
     */
//    public static List<EsSuggestWord> getSuggestWords(String indexName, SuggestBuilder suggestBuilder, BoolQueryBuilder queryBuilder) {
//        SearchRequest searchRequest = new SearchRequest(indexName);
//        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
//        //设置搜索超时的时间
//        searchSourceBuilder.timeout(new TimeValue(60, TimeUnit.SECONDS));
...
//            throw new MyException("搜索出现异常");
//        }
//        List<EsSuggestWord> list = new ArrayList<>();
//        if (searchResponse.status().getStatus() == 200) {
//            Suggest suggest = searchResponse.getSuggest();
//            CompletionSuggestion suggestion = suggest.getSuggestion("my-suggest-1");
//            List<CompletionSuggestion.Entry.Option> options = suggestion.getOptions();
//            for (CompletionSuggestion.Entry.Option option : options) {
//                EsSuggestWord suggestWord = new EsSuggestWord();
//                suggestWord.setProductName(option.getText().toString());
